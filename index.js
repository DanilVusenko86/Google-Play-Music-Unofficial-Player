const { app, BrowserWindow, ipcMain, dialog } = require('electron');
const path = require('path');
const fs = require('fs');
const mm = require('music-metadata');

let mainWindow;

function createWindow() {
    mainWindow = new BrowserWindow({
        minWidth: 1098,
        minHeight: 628,
        width: 1157,
        height: 691,
        transparent: true,
        frame: false,
        icon: path.join(__dirname, 'img', 'icon.ico'),
        autoHideMenuBar: true,
        webPreferences: {
            preload: path.join(__dirname, 'preload.js'),
            nodeIntegration: false,
            contextIsolation: true
        }
    });

    mainWindow.loadFile('index.html');

};

app.whenReady().then(() => {
    createWindow();

    app.on('activate', () => {
        if (BrowserWindow.getAllWindows().length === 0) createWindow();
    });
});

app.on('window-all-closed', () => {
    if (process.platform !== 'darwin') app.quit();
});

// -==============================- \\
//    Scan Folder                   \\
// -==============================- \\

async function scanFolder(folderList, subDir = true) {
    const musicFiles = [];

    for (const folder of folderList) {
        if (typeof folder !== 'string' || folder.trim() === '') {
            console.warn(`Skipping invalid folder: ${folder}`);
            continue;
        }

        try {
            const entries = fs.readdirSync(folder, {
                recursive: subDir,
                withFileTypes: true
            });

            for (const entry of entries) {
                if (entry.isDirectory()) continue;

                const fullPath = path.join(entry.parentPath, entry.name);
                const ext = path.extname(entry.name).toLowerCase();

                if (['.mp3', '.flac', '.wav', '.m4a', '.ogg', '.aac'].includes(ext)) {
                    musicFiles.push(fullPath);
                }
            }
        } catch (err) {
            console.error(`[Scan Folder] Error in ${folder}: ${err.message}`);
        }
    }

    return musicFiles;
}

ipcMain.handle('scanFolder', async (event, folderList, subDir) => {
    return await scanFolder(folderList, subDir);
});


// -==============================- \\
//    Get Metadata                  \\
// -==============================- \\

async function getMetadata(filepath) {
    try {
        const metadata = await mm.parseFile(filepath, {
            duration: true,
            native: true
        });

        let thumbnail = null;
        let DAEFAULT_THUMBNAIL = ""

        if (
            metadata.common.picture && 
            Array.isArray(metadata.common.picture) && 
            metadata.common.picture.length > 0 &&
            metadata.common.picture[0]?.data
        ) {
            thumbnail = Buffer.from(metadata.common.picture[0].data).toString('base64');
        } else {
            thumbnail = DAEFAULT_THUMBNAIL
        }

        let info = {
            path: filepath,
            name: metadata.common.title || path.basename(filepath),
            album: metadata.common.album || path.basename(path.dirname(filepath)),
            year: metadata.common.year || "",
            artist: metadata.common.artist || "Unknown Artist",
            albumartist: metadata.common.albumartist || "Unknown Artist",
            genre: metadata.common.genre?.[0],
            lyrics: metadata.common.lyrics,
            duration: metadata.format.duration,
            thumbnail: thumbnail
        }
        
        return info;

    } catch (err) {
        console.error(`[Get Metadata] Error:`, err.message);
    }
}

ipcMain.handle('getMetadata', async (event, filepath) => {
    return await getMetadata(filepath);
});

// -==============================- \\
//    Select Folder                 \\
// -==============================- \\

async function selectFolder() {
    const { canceled, filePaths } = await dialog.showOpenDialog({
        properties: ['openDirectory'],
        title: 'Choose a folder',
    });

    if (canceled || filePaths.length === 0) {
        return null;
    }

    const normalized = path.posix.normalize(filePaths[0].replace(/\\/g, '/'));

    return normalized;
}

ipcMain.handle('selectFolder', async () => {
    return await selectFolder();
});

// -==============================- \\
//    Window Management             \\
// -==============================- \\

ipcMain.on('window:minimize', () => {
  mainWindow.minimize()
})

ipcMain.on('window:maximize', () => {
  if (mainWindow.isMaximized()) {
    mainWindow.unmaximize()
  } else {
    mainWindow.maximize()
  }
})

ipcMain.on('window:close', () => {
  mainWindow.close()
})